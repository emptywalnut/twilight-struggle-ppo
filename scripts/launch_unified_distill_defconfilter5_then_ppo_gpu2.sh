...
RUN_DIR=/mnt2/users/kaile/hantao/game/struggle/runs/ppo/unified-distill-defconfilter5-gpu${GPU}-${STAMP}
PYTHON=/mnt2/users/kaile/miniconda3/envs/struggle-ppo/bin/python

SOURCE_RUN=/mnt2/users/kaile/hantao/game/struggle/runs/ppo/transformer-history-elo-history3-defconfilter5-300-gpu2-20260627-091331
...
DISTILL_NUM_GPUS="${DISTILL_NUM_GPUS:-1}"
...
"$PYTHON" scripts/generate_policy_distill_warmup.py \
  ...
  --reject-terminal-reason "scoring card held" \
  --reject-terminal-reason "nuclear_war" \
  --reject-terminal-reason "thermonuclear war" \
  --reject-terminal-reason "Cuban Missile Crisis" \
